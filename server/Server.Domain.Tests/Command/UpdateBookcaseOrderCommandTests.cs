...
public class UpdateBookcaseOrderCommandTests(DbFixture fixture) : IClassFixture<DbFixture>
{
    ...
    public async Task Updates_order_of_books_in_bookshelf()
    {
        ...
        var bookshelf = new Bookshelf { Id = bookshelfId, Name = "Reading", CustomerId = customer.Id, CreationDate = DateTimeOffset.UtcNow };

        var bookA = new Book { Isbn = "ISBN-A", Name = "Book A" };
        var bookB = new Book { Isbn = "ISBN-B", Name = "Book B" };

        var customerBookA = new CustomerBook { Id = Guid.NewGuid(), Isbn = bookA.Isbn, Book = bookA, Customer = customer, CustomerId = customer.Id };
        var customerBookB = new CustomerBook { Id = Guid.NewGuid(), Isbn = bookB.Isbn, Book = bookB, Customer = customer, CustomerId = customer.Id };
        ...
        context.BookshelfBook.AddRange(
            new BookshelfBook { BookshelfId = bookshelfId, CustomerBookId = customerBookA.Id, Isbn = bookA.Isbn, Order = 0, Bookshelf = bookshelf, CustomerBook = customerBookA },
            new BookshelfBook { BookshelfId = bookshelfId, CustomerBookId = customerBookB.Id, Isbn = bookB.Isbn, Order = 1, Bookshelf = bookshelf, CustomerBook = customerBookB }
        );
        ...
        await fixture.Execute(
            ...
            new UpdateBookcaseOrderCommand
            {
                ...
                Books = [new UpdateBook { Isbn = bookA.Isbn, Order = 5 }, new UpdateBook { Isbn = bookB.Isbn, Order = 2 }],
            }
        );
